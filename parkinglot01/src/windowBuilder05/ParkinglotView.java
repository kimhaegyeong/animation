...
public class ParkinglotView extends JFrame {
	...
	public ParkinglotView() {
		...
		for (int i = 0; i < parkingLot.getPlateLoation().length; i++) {
			for (int j = 0; j < parkingLot.getPlateLoation()[0].length; j++) {
				if (parkingLot.getPlateLoation()[i][j]) {
					BorderColor BorderColor = new BorderColor(table, i, j, Color.blue, new Insets(0, 0, 3, 0));
				}
			}
		}
		...
		public Component getTableCellRendererComponent(JTable table,
			...
			if (isSelected) {
				...
				if (row > 2 && row < 9 && column > 0 && column < 4) { // A
					...
					cell.setBackground(Color.white);
				} else if (closedDoor == true && row >= 0 && row < 2 && column == 8) { // door
																						// close
	...
	class DoorOpenListener extends DefaultTableCellRenderer implements
		...
		public void actionPerformed(ActionEvent e) {
			...
			BorderColor BorderColor1 = new BorderColor(table, 0, 8, Color.blue, new Insets(0, 3, 3, 3));			
			BorderColor BorderColor2 = new BorderColor(table, 1, 8, Color.blue, new Insets(0, 0, 0, 0));
			table.repaint();
		}
	}
	...
	class DoorCloseListener extends DefaultTableCellRenderer implements
		...
		public void actionPerformed(ActionEvent e) {
			...
			BorderColor BorderColor1 = new BorderColor(table, 0, 8, Color.blue, new Insets(0, 3, 0, 3));
			BorderColor BorderColor2 = new BorderColor(table, 1, 8, Color.blue, new Insets(0, 3, 3, 3));
