...
public class ParkinglotView01 extends JFrame {
	...
	public class CustomTableCellRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table,
			...
			if (isSelected) {
				...
				if (row > 2 && row < 9 && column > 0 && column < 4) { // A
					...
					cell.setBackground(new Color(0x3F3F3F));
						...
						&& column == 8) { // door open
					cell.setBackground(Color.white);
				} else if (row > 2 && row < 8 && column > 3 && column < 8) {      // 주차 공간
					cell.setBackground(new Color(0xC0C0C0));
				} else if (row > 2 && row < 8 && column > 10 && column < 15) {     // 주차 공간
					cell.setBackground(new Color(0xC0C0C0));
				} else
			...
			if (parkingLot.getState()[row][column].getCarNo() == null) {
				...
				setText(parkingLot.getState()[row][column].getCarNo().toString());
			}
	...
	class InputCarListener extends DefaultTableCellRenderer implements
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (inputText.equals("")) {
				...
				parkingLot.getState()[1][9].setCarNo(Integer.parseInt(inputText));	
			
				
				minRouteSpace = parkingLot.findEmptySpace(Integer.parseInt(inputText));
			...
			System.out.println("inputCarNo :" + parkingLot.getState()[1][9].getCarNo());
		}
	...
	class AddCar extends DefaultTableCellRenderer implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			System.out.println(">>>> moveLiftB");
			moveLiftB.setTimer();

			// plate를 liftB로 옮기기
			MovePlate movePlate = new MovePlate(minRoutePlate);
			System.out.println(">>>> movePlate");
			movePlate.setTimer();
		...
		public void setTimer() {
			...
			ActionListener action = new ActionListener() {
				...
				public void actionPerformed(ActionEvent event) {
					...
					System.out.println("+++"
							+ parkingLot.getState()[1][9].getNo());
					System.out.println(parkingLot.getState()[0][9].getNo());

					// 체크
					if (departLocation[0] == 1) {
						System.out.println("!!" + parkingLot.getLiftB()[0]);
					} else {
						System.out.println("@@" + parkingLot.getLiftB()[0]);
					}
				}
	...
	class MovePlate {
		...
		public MovePlate(Plate minRoutePlate) {
			this.minRoutePlate = minRoutePlate;

			seletedPlate = new int[] { minRoutePlate.getRow(),
					minRoutePlate.getColumn() };

			moveingPlate = new int[] { parkingLot.getLiftB()[0],
					parkingLot.getLiftB()[1] };
		}
			...
			ActionListener action = new ActionListener() {
				...
				public void actionPerformed(ActionEvent event) {
					if (moveingPlate[1] == seletedPlate[1]) {
						...
						} else if (seletedPlate[1] > 9) {
							moveingPlate[1]++;
							if (moveingPlate[1] == 10 || moveingPlate[1] == 15) {
								moveingPlate[1]++;
							}
						}
