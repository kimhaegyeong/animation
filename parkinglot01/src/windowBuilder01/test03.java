...
public class test03 extends JFrame {
	...
	public class CustomTableCellRenderer extends DefaultTableCellRenderer {
		public Component getTableCellRendererComponent(JTable table,
			...
			} else if ((row > 1 && row < 9 && column > 7 && column < 11) || row == 1 && column > 8 && column < 11) {   // B
				cell.setBackground(Color.white);
			} else if (row > 2 && row < 9 && column > 14 && column < 18) { // C
				cell.setBackground(Color.white);
			} else if (row >= 0 && row < 2 && column >= 0 && column < 8) { // 지상
				cell.setBackground(Color.white);	
			} else if (row >= 0 && row < 2 && column == 8) {  				// door
				cell.setBackground(Color.gray);
			} else if (row >= 0 && row < 2 && column == 8) {  				// door
				cell.setBackground(Color.gray);
			} else if (row > 2 && row < 8 && column > 3 && column < 8) {  				// door
				cell.setBackground(new Color(0xFAFAFA));
			} else if (row > 2 && row < 8 && column > 10 && column < 15) {  				// door
				cell.setBackground(new Color(0xFAFAFA));
			} else
