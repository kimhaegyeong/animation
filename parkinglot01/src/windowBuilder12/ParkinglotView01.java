...
import windowBuilder12.ParkingLot.Plate;
import windowBuilder12.ParkinglotView01.BorderColor;
...
public class ParkinglotView01 extends JFrame {
	...
	public ParkinglotView01() {
		...
		label_1.setBounds(12, 90, 75, 15);
		panel_2.add(label_1);

		textField = new JTextField();
		textField.setBounds(85, 85, 223, 29);
		panel_2.add(textField);
		textField.setColumns(10);

		JLabel lblParking = new JLabel("주차");
		lblParking.setBounds(12, 140, 60, 15);
		panel_2.add(lblParking);

		JButton btnOut = new JButton("입고");
		btnOut.addActionListener(new InputCarListener());
		btnOut.setBounds(85, 130, 99, 30);
		panel_2.add(btnOut);

		JButton btnIn = new JButton("출고");
		btnIn.addActionListener(new OutCarListener());
		btnIn.setBounds(205, 130, 99, 30);
		panel_2.add(btnIn);
		...
		btnAdd.addActionListener(new AddCarListener());
		btnAdd.setBounds(85, 175, 99, 30);
		panel_2.add(btnAdd);

		JButton btnRemove = new JButton("Remove");
		btnRemove.addActionListener(new RemoveCarListener());
		btnRemove.setBounds(205, 175, 99, 30);
	...
	class InputCarListener extends DefaultTableCellRenderer implements
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (inputText.equals("")) {
				 ...
				 * minRouteSpace); movePlate.setTimer();
				 */
				table.setDefaultRenderer(Object.class,
						new CustomTableCellRenderer());
				table.repaint();
	...
	class OutCarListener extends DefaultTableCellRenderer implements
			ActionListener {
		@Override
		public void actionPerformed(ActionEvent e) {
			JOptionPane.showMessageDialog(null, "자동차 출고");
		}
	}

	class RemoveCarListener extends DefaultTableCellRenderer implements
			ActionListener {
		@Override
		public void actionPerformed(ActionEvent e) {
			JOptionPane.showMessageDialog(null, "자동차 제거");
		}
	}

	class AddCarListener extends DefaultTableCellRenderer implements
			ActionListener {
		@Override
		public void actionPerformed(ActionEvent e) {
			car = new int[] { 1, 5 };
			...
			Timer timerLiftB = new Timer(500, new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (moveLiftB.setTimer() == true) {
						((Timer) e.getSource()).stop();
					}
					lock.countDown(); // Signal to the CountDownLatch that
										// this timer is finished
				}
			});
			timerLiftB.setRepeats(false);
			timerLiftB.start();
			...
			MovePlate movePlate = new MovePlate(minRoutePlate,
					parkingLot.getLiftB());
			Timer timerPlate = new Timer(500, new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (movePlate.setTimer() == true) {
						((Timer) e.getSource()).stop();					
					}
					lock.countDown(); 
				}
			});
			timerPlate.setRepeats(false);
			timerPlate.start();

			// F1 층으로 올라가기
			System.out.println(">>>> gotoF1Lift");
			MoveLift gotoF1Lift = new MoveLift("F1");
			Timer timerGotoF1 = new Timer(500, new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (gotoF1Lift.setTimer() == true) {
						((Timer) e.getSource()).stop();
					}
					lock.countDown(); 
				}
			});
			timerGotoF1.setRepeats(false);
			timerGotoF1.start();
			System.out.println("AddCarListener end");
		}
	}
	...
	class MoveLift {
		...
		public boolean setTimer() {
			...
			if (i > departLocation[0]
				...
				liftRendering(temp + 1, parkingLot.getLiftB()[1], new Insets(0,
						0, 0, 0));
				temp = i - 1;
				liftRendering(temp + 1, parkingLot.getLiftB()[1], new Insets(
						15, 0, 0, 0));
				...
				liftRendering(temp, parkingLot.getLiftB()[1], new Insets(0, 0,
						0, 0));
				temp = i + 1;
				liftRendering(temp + 1, parkingLot.getLiftB()[1], new Insets(
						15, 0, 0, 0));
			...
			if (empty == false) {
				movePlate.PlateRendering(parkingLot.getLiftB()[0],
						parkingLot.getLiftB()[1], new Insets(0, 0, 3, 0));

				try {
					parkingLot.getState()[temp][parkingLot.getLiftB()[1]] = (Plate) parkingLot
							.getState()[i][parkingLot.getLiftB()[1]].clone();
				} catch (CloneNotSupportedException e) {
					e.printStackTrace();
				}
				parkingLot.getState()[i][parkingLot.getLiftB()[1]].clear();
			}
	...
	class MovePlate {
		...
		public boolean setTimer() {
			...
			System.out.println("parkingLot.getLiftB() : "
					+ parkingLot.getLiftB()[0] + ", "
			...
			if (moveingPlate[1] == seletedPlate[1]) {
				...
				if (seletedPlate[1] - temp[1] < 0) {
					moveingPlate[1]--;
					if (moveingPlate[1] == 3 || moveingPlate[1] == 8
							|| moveingPlate[1] == 10 || moveingPlate[1] == 15) {
						moveingPlate[1]--;
					}
				...
				PlateRendering(moveingPlate[0], moveingPlate[1], new Insets(0,
						0, 0, 0));

				PlateRendering(temp[0], temp[1], new Insets(0, 0, 3, 0));
				...
				parkingLot.getState()[moveingPlate[0]][moveingPlate[1]].clear();
			}
			return false;
		}
