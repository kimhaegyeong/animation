...
public class ParkinglotView02 extends JFrame {
	...
	class AddCar extends DefaultTableCellRenderer implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			// liftB 옮기기
			MoveLift moveLiftB = new MoveLift(minRoutePlateNo);
			moveLiftB.setTimer();
			
			// plate를 liftB로 옮기기
			MovePlate movePlate = new MovePlate(minRoutePlateNo);
			movePlate.setTimer();
			
			// F1 층으로 올라가기
			MoveLift gotoF1 = new MoveLift("F1");
			gotoF1.setTimer();
		}
	}
	...
	class MoveLift {
		...
		public MoveLift(int minRoutePlateNo) {
			//this.minRoutePlateNo = minRoutePlateNo;
			departLocation[0] =  parkingLot.getPlates()[minRoutePlateNo].getRow();
			departLocation[1] = parkingLot.getPlates()[minRoutePlateNo].getColumn();
		}
		
		public MoveLift(String F1) {
			//F1Location = new int[]{2, 9};
			departLocation[0] = 2;
			departLocation[1] = 9;
		}
		...
		public void setTimer() {
			...
			ActionListener action = new ActionListener() {
				...
				public void actionPerformed(ActionEvent event) {
					liftRendering(parkingLot.getLiftB(), new Insets(0, 0,
							0, 0));
					parkingLot.setLiftB(i, 9);
					liftRendering(parkingLot.getLiftB(), new Insets(15, 0,
							0, 0));
					
					if (i >= parkingLot.getPlates()[minRoutePlateNo].getRow() && 
							(parkingLot.getLiftB()[0] - parkingLot.getPlates()[minRoutePlateNo].getRow()) > 0 ) {
						i--;
					} else if (i <= parkingLot.getPlates()[minRoutePlateNo].getRow() && 
							(parkingLot.getLiftB()[0] - parkingLot.getPlates()[minRoutePlateNo].getRow()) > 0 ) {
						i++;
					}  
					else {						
		...
		void liftRendering(int[] lift, Insets inset) {
			BorderColor Renderingring = new BorderColor(table, lift[0] + 1,
					lift[1], Color.red, inset);
		}
	}

	class MovePlate {
		private Timer timer;
		private int minRoutePlateNo;
		private int row;
		private int column;

		public MovePlate(int minRoutePlateNo) {
			this.minRoutePlateNo = minRoutePlateNo;
			row = parkingLot.getPlates()[minRoutePlateNo].getRow();
			column = parkingLot.getPlates()[minRoutePlateNo].getColumn();
		}
		...
		public void setTimer() {
			ActionListener action = new ActionListener() {
				...
				@Override
				public void actionPerformed(ActionEvent event) {
					if (i > parkingLot.getLiftB()[1]) {
						timer.stop();

					} else {
						PlateRendering(row, column, new Insets(0, 0, 0, 0));

						if (column == 7) {
							PlateRendering(row, column + 2, new Insets(0, 0, 3,
									0));
						} else {
							PlateRendering(row, column + 1, new Insets(0, 0, 3,
									0));
						}
						i++;
					}
